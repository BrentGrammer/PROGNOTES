...
- Access parts of large objects with SQL like syntax
- Prevents having to download large objects and then filter on the client (saves transfer cost and time)
...
- Allows for Event driven workflows and notifications when certain things happen in S3
  - Can send to SQS, SNS, or delivered to Lambda Functions
- Send events when Objects Created (Put, Post, Copy, CompleteMultiPartUpload)
- Send events when Objects Deleted (\*, Delete, DeleteMarkerCreated)
  - example use case: notification for security if any objects are removed from the S3 Bucket
- Trigger on Object Restores (Post(Initiated), Completed)
  - example use case: notify staff or customers when a restore of Glacier or S3 data begins or ends
- Trigger on Replication events (OperationMissedThreshold, OperationReplicatedAfterThreshold, OperationNotTracked, OperationFailedReplication)
  - i.e. notification if objects are not replicated within the 15 minute threshold or fails being replicated

### Event Notification Configuration

- Create a Event Notification Config that is attached to the S3 bucket to define which events to send where
  - Can send to Lambda, SNS Queues or SNS Topics
- Events are generated by the S3 Principal
  - Need to add Resource Policies on each of the destination services to allow the S3 service to interact with them
    - i.e. an SQS Queue policy, or a Lambda Resource Policy etc.
...
// example event
"s3": {
  "configurationId": "ID in the bucket notification configuration",
  "bucket": {
    "name": "bucketname",
    "ownerIdentity": {
      "principalId": "Amazon-customer-ID-of-Bucket-Owner"
    },
    "arn": "bucket:arn::"
  },
  "object": {
...
- These are sent in the event to a Lambda, for example and then you parse what you need from them.

### Preferred Alternative: EventBridge

- Event notifications are an old feature of AWS and are limited in the services they work with
- EventBridge is another service that supports more types of events and services to interact with
- **By default, just use EventBridge for event driven workflows instead of Event Notifications unless you have a specific reason not to do so**
...
- Understand access patterns of users
- Understand charges on S3 bill

### Source and Target Buckets

- Monitor a **Source Bucket** to understand what types of access are occuring on it, and send the logs to a **Target Bucket**
  - A Single Target Bucket can accept logs from many Source Buckets, collected using different prefixes
  - Set the prefix for logging in the config on the Source Bucket
- Enable logging on the Source Bucket via the console or the CLI
- The **S3 Log Delivery Group** reads the configuration you set on the Source Bucket
  - Need to grant access to the S3 Delivery Log Group on the Target Bucket using an ACL on that bucket
    - \*Add the S3 Delivery Group and give it Write Access
- Best effort process - it can take a few hours for the logging to take affect after enabling

### Logs

- Delivered as Log files
- Each file consists of log records which are new-line delimited
  - Operator, Date and Time, the Operation, Status Code, Error Code, etc.
  - The attributes on each log record are SPACE delimited
- Lifecycle management required! manage the deletion of files or moving to different storage classes manually!

## S3 Object Lock

- Can ONLY enable for NEW S3 Buckets!
  - If you want to enable it later, need to contact AWS Support
  - You CANNOT disable object lock or suspend versioning after it is set
- When Creating with object lock, S3 Versionining is also enabled on the bucket and is required
- WRITE-ONCE-READ-MANY Architecture ("WORM") - Once objects are created, they cannot be overwritten or deleted.
  - Individual Object versions are locked.

### Retention Management for S3 Object Lock

- Two types of retention management (can have both applied, or one or the other)
  - S3 Object Lock Retention Period
  - S3 Object Lock Legal Hold
- Can defined these on individual object versions, or on bucket defaults

### Retention Period setting

- Specify period in days/years when you set the Object Lock
- Two Modes (**IMPORTANT**):

  - Compliance Mode: the object cannot be deleted or overwritten for the duration of the retention period.
    - Also means that the retention period cannot be adjusted or changed during the period
    - No changes are allowed to the Object version itself either
      - Even by the Account ROOT user - cannot make ANY changes
    - Most restrictive setting - do not set this if you do not need the object to remain absolutely unaltered during the retention period - used for compliance, medical/financial data, etc.
  - Governance Mode: less strict than Compliance mode.
    - Still restricts changes to object versions, deleting etc. for a given retention period, but allows special permissions for certain users to make changes to allow deleting or updating the object during the retention period.
      - Permission: `s3:BypassGovernanceRetention`
      - Pass header with request: `x-amz-bypass-governance-rentention:true`
        - Note: this is the default header sent in the AWS console UI - if you have the permission above as well, then you can make changes in the console.
    - Useful to prevent accidental deletion, can use it as a test for settings before picking Compliance Mode.

  ### Legal Hold

  - No setting a retention period. A binary setting that is either on or off
  - While enabled on a specific Object version, you can't delete or overwrite it
  - Permission required: `s3:PutObjectLegalHold`
    - required to add or remove the legal hold feature
    - status=ON
    - when set to OFF, normal permissions apply and the object can be replaced or deleted as required
  - Useful to prevent deletions or flag a specific object version that is critical for a given project.

# Example IAM permissions for a sensitive bucket:

- Also consider
  - Secure Transport must be on (https)

1. Use IAM Groups for Users

Create an IAM group, e.g. S3AdminGroup, and attach a policy like this:
...
Then assign your admin users to this group.

2. Use IAM Roles for EC2 Access

Create an IAM Role (e.g. S3AccessRole) with the same S3 access policy as above.

Attach that role to your EC2 instance.

The EC2 app will automatically get credentials from the instance metadata and use them via the AWS SDK.

3. Secure the S3 Bucket Policy

To enforce access control at the bucket level, use a bucket policy that:

Allows access to:

The specific IAM role used by EC2.

The S3AdminGroup users (via aws:PrincipalArn condition or wildcard).

Denies everything else.

Here's an example of a restrictive bucket policy:
...
4. (Optional) VPC Endpoint for S3

To restrict access to S3 only via your VPC, and avoid data leaving over the public internet:

Create a VPC Gateway Endpoint for S3.

Update your bucket policy to allow only requests coming from your VPC endpoint:
