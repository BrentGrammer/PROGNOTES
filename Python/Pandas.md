...
- Beware that df.count() will only return the count of non-NA/NaN rows for each column. You should use df.shape[0] instead, which will always correctly tell you the number of rows.
...
- df[df['Credit Card']==4926535242672853]['Email'].to_string(index=False)
...
- Use apply() with a predicate:

```python
def pred(coldata):
  return 'needle' in coldata.lower()

df['Col'].apply(lambda x: pred(x))
```
